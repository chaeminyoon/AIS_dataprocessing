...
DAILY_SHAPEFILES_DIR = os.path.join(OUTPUT_DIR, "daily_shapefiles")

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(DAILY_RESULTS_DIR, exist_ok=True)
os.makedirs(DAILY_SHAPEFILES_DIR, exist_ok=True)
...
def process_single_day(args):
    ...
    try:
        ...
        # 4. 잘라진 궤적을 Shapefile로 저장
        shapefile_path = os.path.join(DAILY_SHAPEFILES_DIR, f"clipped_trajectories_{day_num}.shp")
        ...
        print(f"[Day {day_num}] Saved shapefile: {os.path.basename(shapefile_path)}")

        # 5. 길이 계산 (WGS84 → Web Mercator)
