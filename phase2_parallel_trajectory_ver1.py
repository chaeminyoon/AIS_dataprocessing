...
def intersect_with_grid(gdf_trajectories, gdf_grid):
    """Intersect trajectories with grid cells - optimized version."""
    results = []
    spatial_index = gdf_grid.sindex
    total_trajectories = len(gdf_trajectories)
    
    for idx, traj_row in gdf_trajectories.iterrows():
        # Progress indicator every 100 trajectories
        if (idx + 1) % 100 == 0:
            print(f"    [{idx + 1}/{total_trajectories}] trajectories processed...")
        
        ship_id = traj_row['ship_id']
        trajectory = traj_row['geometry']
        
        # Get candidate grids using spatial index (bounding box)
        ...
        # First filter: quick bounds check
        bounds = trajectory.bounds  # (minx, miny, maxx, maxy)
        
        # Only check grids whose bounds actually overlap with trajectory bounds
        overlapping = possible_matches[
            (possible_matches.bounds.minx <= bounds[2]) &  # grid minx <= traj maxx
            (possible_matches.bounds.maxx >= bounds[0]) &  # grid maxx >= traj minx
            (possible_matches.bounds.miny <= bounds[3]) &  # grid miny <= traj maxy
            (possible_matches.bounds.maxy >= bounds[1])    # grid maxy >= traj miny
        ]
        
        if overlapping.empty:
            continue
        
        # Precise intersection check (expensive operation)
        for _, grid_row in overlapping.iterrows():
